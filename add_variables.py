...
recession_dummy['observation_date'] = pd.to_datetime(recession_dummy['observation_date'], utc=True).dt.normalize()
...
all_df_of_close_data['high_frequency_trader'] = (all_df_of_close_data['filing_count_reportingOwner.name'] > all_df_of_close_data['filing_count_reportingOwner.name'].median()).astype('int8')
...
# Clusterbuy dummy
all_df_of_close_data['clusterbuy'] = (all_df_of_close_data['trades_14d'] > 1).astype('int8')
...
all_df_of_close_data = all_df_of_close_data[all_df_of_close_data['holding_change_percent']>=0]

# high freqency trader
all_df_of_close_data['high_frequency_trader'] = (all_df_of_close_data['filing_count_reportingOwner.name'] > all_df_of_close_data['filing_count_reportingOwner.name'].median()).astype('int8')

# high change in holdings dummy
all_df_of_close_data['high_change_in_holdings'] = (all_df_of_close_data['holding_change_percent'] > all_df_of_close_data['holding_change_percent'].median()).astype('int8')
...
for p in range(1, 11):
    target = all_df_of_close_data[target_help_cols].ge(p).astype('int8')  # .gt() -> >=
