...
def stats(df):
    """
    Adds isna(), skew(), and var() on top of describe(). An overview of the
    results is saved in the export folder.
    skew() in Excel = SKEW(A2:A11)
    var() is equivalent to VAR.S in Excel for binary variables and VAR.P for
    ...
    Output:
    pd.DataFrame: overview with descriptive statistics and missing values for
    each variable
    ...
    skew = pd.DataFrame(df.select_dtypes(include=[np.number]).skew(),
                        columns=["skew"])

    # compute variance for numeric columns only
    var = pd.DataFrame(df.select_dtypes(include=[np.number]).var(),
                       columns=["var"])
    ...
    # change float display format, variances are shown in scientific notation
    descriptive['var'] = descriptive['var'].apply(
...
def clean_dtypes(df):
    ...
    Input:
        df (pd.DataFrame): our current data
        max_unique_cats (int): Maximum number of categories for one-hot
    ...
    # all() is True if all items in an iterable are true
    for col in df.columns:
        if df[col].nunique(dropna=False) == 2 and df[col].dropna()\
                                                         .isin([0, 1])\
                                                         .all():
    ...
    # if there are string columns, apply one-hot encoding
    if string_columns:  # get_dummies() converts categorical variables in bools
...
def missing_values_summary(df):
    ...
    Output:
        New df (pd.DataFrame): variable names with NA count and percentage
    ...
    for col in df.columns:
        if df[col].isna().any():
            if col in except_replace_0:
                ...
                # It flips 1's with 0's and vice versa
...
def remove_correlation(df,
    ...
    All variables with a correlation above the threshold (default = 0.8) are
    removed according to the following decision rule:
    If two variables correlate above the threshold, the one with the lower
    correlation to the target variable is removed. The idea is to keep the
    variable with the higher correlation to the target because it provides
    more explanatory power for the model. The output includes a list of
    removed variables with their correlations and the cleaned DataFrame.

    To verify in Excel: =CORREL(--bool_variable_1, --bool_variable_2).
    Use the double unary (--) for boolean values. If missing values are not
    handled beforehand, Excel may yield slightly different correlations. If
    you pass the data with missing values already handled, Excel will produce
    the same correlations as Python.
    ...
    Output:
        New df (pd.DataFrame): without variables whose correlation exceeds
        correlation_threshold
        Correlation matrix (Excel file): saved in the notebook directory
        Print statements: list of all dropped variables with correlation
        details
    """
    ...
    for i in range(len(corr_matrix.columns)):  # range up to the number of cols
        # range(i) yields 0..i-1; e.g., i=0 -> [], i=1 -> [0], i=2 -> [0, 1]
        # i=5 -> [0, 1, 2, 3, 4]
        for j in range(i):
            ...
            if corr_value > correlation_threshold:
                # skip the next part if in to_drop or the target
                if var1 in to_drop or\
                   var2 in to_drop or\
                   var1 == target_col or\
                   var2 == target_col:
                    continue
                ...
                var1_target_cor = corr_with_target.get(var1, 0)
                var2_target_corr = corr_with_target.get(var2, 0)

                # compare their correlations with the target
                if var1 and var2 in never_drop:  # check if in never_drop
                    continue

                # if var2 in never_drop or
                # var2's correlation with the target is higher then
                # keep var2 and drop var1
                elif var2 in never_drop or var1_target_cor < var2_target_corr:
                    remove_var = var1
                    keep_var = var2
                    remove_corr = var1_target_cor
                    keep_corr = var2_target_corr

                # if var1 is in never_drop or
                # var1's correlation with the target is higher then
                # keep var1 and drop var2
                elif var1 in never_drop or var2_target_corr <= var1_target_cor:
                    remove_var = var2
                    keep_var = var1
                    remove_corr = var2_target_corr
                    keep_corr = var1_target_cor

                to_drop.add(remove_var)
                removed_details.append((remove_var,
                                        keep_var,
                                        corr_value,
                                        remove_corr,
                                        keep_corr))
    ...
    if not to_drop:
        ...
        for (
            ...
            print(
                (
                    f"In {df_name} was '{remove_var}' removed because of the "
                    f"correlation with '{keep_var}' (r = {corr_value:.3f})"
                )
            )
            print(
                (
                    f"Correlation of '{remove_var}' with '{target_col}': "
                    f"{remove_corr:.3f}"
                )
            )
            print(
                (
                    f"Correlation of '{keep_var}' with '{target_col}': "
                    f"{keep_corr:.3f}"
                )
            )
            print("-" * 30)
...
def variance(df,
    ...
    Input:
        ...
        target_corr_threshold (float): correlation-with-target threshold for
        dropping variables
        df_name (str): name of the DataFrame (e.g., T1_df, T2_df, or diff_df);
        used for log documentation
    ...
    Output:
        New df (pd.DataFrame): without variables whose variance is below
        variance_threshold
        Print statements: list of all removed variables
    """
    # ddof=1 for sample variance; 0 would be population variance
    variances = df.var(skipna=True, ddof=1)

    low_variance_columns = list(variances[(
        variances < variance_threshold)].index)
    ...
    for var in low_variance_columns:
        ...
        if var_target_corr > target_corr_threshold:
            print(f"In {df_name} has '{var}' a low variance of "
                  f"{variances[var]:.6f}, but a high correlation with "
                  f"{target_col} {var_target_corr:.3f} and is not removed from"
                  " the df")
    ...
    if not to_drop:
        print(f"In {df_name} has no low variance variables and no columns were"
              " removed.")
    else:
        print(f"Removed columns in {df_name} with low variance and low "
              f"correlation with {target_col}:")
        for col in to_drop:
            print(f"{col}: Variance = {variances[col]:.6f}, correlation with "
                  f"{target_col} = {corr_with_target[col]:.3f}")
    print("-" * 80)
...
def count_outliers(df, columns=None, iqr_k=1.5):
    """
    Makes table summary of amount of outliers in df. No strings no bools in df
    """
...
# count_outliers(relevant_data_uncleaned.select_dtypes(exclude="bool"),
# columns=None,
# iqr_k=1.5)
...
def outlier_strategy_comparison(df, name_target="target_variable"):
    ...
    for var in variables:
        ...
        count = count_outliers(df, column=var)
        fig.subtitle(f"Outlier strategy for {var} (Outliers: {count}",
                     fontsize=16)
...
def transform_skewness(df, variable=None, transformation=None):
    ...
    Notes:
        ...
        - power_transform refers to scikit-learns Yeo-Johnson, works with
         zeros and negatives

    Output:
        pd.DataFrame: DataFrame with the column transformed and renamed
    ...
    print(f"Skewness of '{variable}' was before {og_skewness} and "
          f"{transformation} is {new_skewness}")
...
def standardization(df, scale_strategy="all", columns_to_scale=None):
    ...
    Recommended for: logistic regression and other linear models; typically
    not needed for tree-based models (Decision Trees, Random Forest, XGBoost).
    Prefer running transform_skewness first, then applying the Z-score.

    Input:
        df (pd.DataFrame): original DataFrame
        scale_strategy (str): "all" (scale all non-boolean columns) or
        "only_chosen_variables" (scale only selected columns)
        variable (list[str]): list of column names to scale when using
        "only_chosen_variables", e.g., ["Variable_1"]

    Output:
        pd.DataFrame: DataFrame with standardized variables (where applicable)
        print: information about which variables were scaled
        pd.DataFrame via stats(df): overview with descriptive statistics and
    ...
    if scale_strategy == "all":
        ...
        for col in columns:
            ...
            print(f"{col} were adjusted with z score")
...
def null_analyse(df):
    ...
    What it does:
        - Computes the overall percentage of zeros for each col
        - Computes, among rows where target == 1, the percentage of zeros
        - Returns a summary DataFrame sorted by overall zero percentage.
        - Flags "critical" variables default rule:
        >99% zeros overall AND >99% zeros when target == 1).
    ...
    Notes:
        ...
        - If there are no rows with target == 1, the target-specific
    ...
    zerodf["critical variable"] = (
        ...
        &
        (zerodf['How many percent of the target = 1 have 0 in this variable']
         > 99)
    )

    # crosstab for critical variables
    critical_vars = zerodf.loc[zerodf["critical Variable"], 'Variable']\
        .tolist()
